    ...
    public class Native
    {
        ...
        void TraceBufRead()
        {
            try
            {
               ...
               // System.Windows.MessageBox.Show("TraceBufRead ");
                while (true)
                {
                    uint bytesReturned;
                    ...
                    if (!result)
                    {
                        ...
                        if (err == ERROR_IO_PENDING)
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine(err);//set indication Lamp for no Input
                    }
                    if (result)
                    {
        ...
        uint IOCTL_BUSDOG_SET_BUFFER = CTL_CODE(FILE_DEVICE_BUSDOG,
                                          2048,
                                          METHOD_BUFFERED,
                                          FILE_WRITE_ACCESS);
        ...
        public bool GetDeviceList(out List<DeviceId> deviceIds)
        {
            ...
            if (result)
            {
                ...
                {
                    BUSDOG_DEVICE_ID devId =(BUSDOG_DEVICE_ID) Marshal.PtrToStructure(new IntPtr(outBuffer.ToInt64() + index),
